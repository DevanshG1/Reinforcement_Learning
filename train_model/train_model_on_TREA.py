...
def main():
    ...
    wandb.init(project = 'rlproject', name = 'experiment3', config=argus)
    wandb.config.update(argus)
        ...
        for t in range(argus.train.per_episode_length):
            ...
            if done or (t >= argus.train.per_episode_length - 1):
                ...
                if (ep+1)%argus.evaluate.eval_every_n_ep == 0:
                    ...
                    mean_episode_rewards = np.mean(eval_episode_rewards)
                    wandb.log({'mean_episode_rewards' : mean_episode_rewards})
                ...
                if (ep + 1) % argus.control.update_every_n_ep == 0:
                    ...
                    for _ in range(argus.control.update_repeat_n_time):
                        ...
                        wandb.log({'critic_loss' : np.mean(critic_loss), 'actor_loss' : np.mean(actor_loss), 'reward_loss' : np.mean(reward_loss)})
